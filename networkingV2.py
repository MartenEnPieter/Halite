...
def deserializeInitMap(serializedMap, myId):
    splitString = serializedMap.split(" ")
    m = GameMap(_width, _height)
    myLocations = []
    y = 0
    x = 0
    ...
    while y != m.height:
        ...
        for a in range(0, counter):
            m.contents[y][x].owner = owner
            if owner == myId:
                myLocations.append(Location(x,y))                 
            x += 1
            if x == m.width:
                x = 0
                y += 1
...
def getInit(myPlayerId):
    deserializeMapSize(getString())
    deserializeProductions(getString()) 
    return deserializeInitMap(getString(), myPlayerId)
